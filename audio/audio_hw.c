...
#define MIXER_SPEAKER_PLAYBACK_SWITCH       "Internal Speaker Switch"
#define MIXER_MIC_CAPTURE_SWITCH            "Internal Mic Switch"

/* ALSA card */
#define CARD_SHUTTLE 0
...
struct shuttle_audio_device {
    ...
    int devices;
    int in_call;
    struct shuttle_stream_in *active_input;
    struct shuttle_stream_out *active_output;
    bool mic_mute;
    struct echo_reference_itfe *echo_reference;
};

struct shuttle_stream_out {
    ...
    int standby;
    struct echo_reference_itfe *echo_reference;
    struct shuttle_audio_device *dev;
    int write_threshold;
};

#define MAX_PREPROCESSORS 3 /* maximum one AGC + one NS + one AEC per input stream */

struct shuttle_stream_in {
...
static int do_input_standby(struct shuttle_stream_in *in);
static int do_output_standby(struct shuttle_stream_out *out);
...
static int start_output_stream(struct shuttle_stream_out *out)
{
    struct shuttle_audio_device *adev = out->dev;
    unsigned int card = CARD_SHUTTLE;
...
static void add_echo_reference(struct shuttle_stream_out *out,
...
static void remove_echo_reference(struct shuttle_stream_out *out,
                                  struct echo_reference_itfe *reference)
{
    pthread_mutex_lock(&out->lock);
...
static void put_echo_reference(struct shuttle_audio_device *adev,
                          struct echo_reference_itfe *reference)
{
    if (adev->echo_reference != NULL &&
...
static struct echo_reference_itfe *get_echo_reference(struct shuttle_audio_device *adev,
...
static int get_playback_delay(struct shuttle_stream_out *out,
                       size_t frames,
...
static int start_input_stream(struct shuttle_stream_in *in)
{
    int ret = 0;
    struct shuttle_audio_device *adev = in->dev;
...
static void get_capture_delay(struct shuttle_stream_in *in,
                       size_t frames,
                       struct echo_reference_buffer *buffer)
{
...
static int32_t update_echo_reference(struct shuttle_stream_in *in, size_t frames)
{
    struct echo_reference_buffer b;
    b.delay_ns = 0;
...
static void push_echo_reference(struct shuttle_stream_in *in, size_t frames)
{
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    in = (struct shuttle_stream_in *)((char *)buffer_provider -
                                   offsetof(struct shuttle_stream_in, buf_provider));
...
static ssize_t read_frames(struct shuttle_stream_in *in, void *buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
static ssize_t process_frames(struct shuttle_stream_in *in, void* buffer, ssize_t frames)
{
    ssize_t frames_wr = 0;
...
{
    struct shuttle_audio_device *ladev = (struct shuttle_audio_device *)dev;
    struct shuttle_stream_out *out;
    int ret;
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    in = (struct shuttle_stream_in *)calloc(1, sizeof(struct shuttle_stream_in));
    if (!in)
        return -ENOMEM;
                     ...
                     hw_device_t** device)
{
    ...
    adev = calloc(1, sizeof(struct shuttle_audio_device));
    if (!adev)
        return -ENOMEM;
